...
def cnn_classifier_page():
    st.header("👚 CNN Fashion Image Classifier")

    # Load CNN model and class indices
    cnn_model = tf.keras.models.load_model('cnn_fashion_model.keras')
    with open('class_indices.json', 'r') as f:
        cnn_class_indices = json.load(f)
    ...
    if uploaded_image:
        ...
        if st.button("Classify Single Image"):
            prediction = predict_image(cnn_model, uploaded_image, cnn_class_indices)
    ...
    def predict_zip_images(zip_file):
        with zipfile.ZipFile(zip_file, "r") as zip_ref:
            ...
            for image_file in image_files:
                ...
                prediction = predict_image(cnn_model, img, cnn_class_indices)
